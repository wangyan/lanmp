...
/* 程序开发: Yahei.Net
/* 联系方式: info@Yahei.net
/* Date: 1970-01-01 / 2011-11-4
/* ---------------------------------------------------- */
/* 感谢以下朋友为探针做出的贡献:
/* zyypp,酷を龙卷风,龙智超,菊花肿了,闲人,Clare Lou,hotsnow,二戒
/* 您可能是下一个?
/* ---------------------------------------------------- */
...
function isfun($funName = '')
{
	...
	return (false !== function_exists($funName)) ? '<font color="green">√</font>' : '<font color="red"><font color="red">×</font></font>';
...

{
    ...
    //$res['memRealUsed'] = ($res['memTotal'] - $res['memFree'] - $res['memCached']); //真实内存使用
	//$res['memRealUsed'] = $res['memUsed'] - $res['memCached']; //真实内存使用
	$res['memRealUsed'] = round(($buf[1][0]-$buf[2][0]-$buf[3][0])/1024,2);
	...
	$res['memCachedPercent'] = (floatval($res['memCached'])!=0)?round($res['memCachedUsed']/$res['memCached']*100,2):0; //cache内存使用率
...
else
{
	...
	$mcu = round($sysInfo['memCachedUsed']/1024,3)." G";
	$mcf = round($sysInfo['memCachedFree']/1024,3)." G";
...
body{text-align: center; margin: 0 auto; padding: 0; background-color:#EEE; font-size:12px; font-family: "Microsoft Yahei", Tahoma, Arial; }
...
table{clear:both;padding: 0; margin: 15px 0 5px; border-collapse:collapse; border-spacing: 0;}

th{padding: 3px 6px; font-weight:bold; background:#FFF; color:#888; border:1px solid #CCC; text-align:left;}
...
.barli_1{background:#FFB90E; height:5px; margin:0px; padding:0;}
...
#footer {padding: 15px 0 30px; text-align: center; font-size: 12px; font-family: "Microsoft Yahei", Tahoma, Verdana;}
...
<table width="100%" cellpadding="3" cellspacing="0">
  ...
  <tr>
    ...
    <td width="37%"><?$os = explode(" ", php_uname());?><?=$os[0];?> &nbsp;内核版本： <?=$os[2]?></td>
...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  <tr>
    ...
    <td width="37%" colspan="3"><span id="uptime">00天00小时00分钟</span></td>
  ...
  <tr>
    ...
    <td width="37%"><?php echo $sysInfo['cpu']['model'];?></td>

    <td width="13%">CPU二级缓存</td>

    <td width="15%"><?php echo $sysInfo['cpu']['cache'];?></td>

    <td width="12%">系统Bogomips</td>

    <td width="10%"><?=$sysInfo['cpu']['bogomips']?></td>
...
&copy; <?php echo date("Y",time());?> PHP探针 Yahei.Net <?php echo $version; ?> All Rights Reserved.<br />
